...
```sh
npm install @anchor-lang/anchor
```
...
keep using the local validator, pass `--provider.cluster localnet` or set it in
`Anchor.toml`.
...
been removed. Use `declare_program!` for cross-program interface definitions
instead.

### `Anchor.toml` cleanup

- The `[registry]` section is no longer recognised and should be removed.
...
test template by default, replacing the previous `anchor-tests` default. You
can still pick a different template explicitly:

```sh
anchor init my-program --test-template mollusk
anchor init my-program --test-template anchor
```
