...
**`errors` module renamed to `error`**:

```diff
- use my_program::errors::MyError;
+ use my_program::error::MyError;
```

**`ProgramError` renamed to `<ProgramName>Error`**, where `<ProgramName>` is
the camel-cased name of the declared program:

```diff
- my_program::ProgramError::SomeVariant
+ my_program::MyProgramError::SomeVariant
```
